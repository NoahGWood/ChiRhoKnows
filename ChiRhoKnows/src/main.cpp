...
void SentencePieceTokenizerExample() {
  ...
  // Read blob from file.
  auto blob = LoadBytesFromFile("models/dist/tokenizer.model");
...
void HuggingFaceTokenizerExample() {
  ...
  // Read blob from file.
  auto blob = LoadBytesFromFile("models/dist/tokenizer.json");
  // Note: all the current factory APIs takes in-memory blob as input.
  // This gives some flexibility on how these blobs can be read.
...
// - models/dist/tokenizer_model
void RWKVWorldTokenizerExample() {
  std::cout << "Tokenizer: RWKVWorld" << std::endl;

  auto start = std::chrono::high_resolution_clock::now();

  auto tok = Tokenizer::FromBlobRWKVWorld("models/dist/tokenizer_model");
